...
"Date,Time,Hostname,Message,Category,ResultCode" > "$Logpath\SCCM_Agent_Install.csv"
"$date,$time,$env:COMPUTERNAME,Collecting Configuration Manager Installation Info,Information,0" >> "$Logpath\SCCM_Agent_Install.csv"
...
foreach ($com in $comp) {
    ...
    if (-not !$source) {
        ...
        if ($CurrentVersion -lt $CompareVersion.build) {
            try {
                "$date,$time,$env:COMPUTERNAME,CCMsetup.exe File Copy to Temp Folder,information,0" >> "$Logpath\SCCM_Agent_Install.csv"
                ...
                try {
                    ...
                    "$date,$time,$env:COMPUTERNAME,SCCM Agent Install Compleate,Information,0" >> "$Logpath\SCCM_Agent_Install.csv"
                }
                catch {
                    "$date,$time,$env:COMPUTERNAME,Error : $($_.Exception.Message),InstallFail from Setup Loop,20" >> "$Logpath\SCCM_Agent_Install.csv"    
                    exit
                }
            }
            catch {
                "$date,$time,$env:COMPUTERNAME,Error : $($_.Exception.Message) from File Copy Action,CopyError,10" >> "$Logpath\SCCM_Agent_Install.csv"
                exit
            }
            
        }
        else {
            "$date,$time,$env:COMPUTERNAME,SCCM Agent Version is Newest ,VersionEnd ,100" >> "$Logpath\SCCM_Agent_Install.csv"
            $ReInstall = 0
        }
    }
    else {
        "$date,$time,$env:COMPUTERNAME,SCCM Agent Not Installed Check,information,0" >> "$Logpath\SCCM_Agent_Install.csv"
        try {
            "$date,$time,$env:COMPUTERNAME,CCMsetup.exe File Copy to Temp Folder,information,0" >> "$Logpath\SCCM_Agent_Install.csv"
            ...
            "$date,$time,$env:COMPUTERNAME,Error : $($_.Exception.Message) from NotInstalled to Setup Action,InstallFail,20" >> "$Logpath\SCCM_Agent_Install.csv"    
            exit
        }
    }
    ...
    if (!$ccmevalLog) {
        try {
            if ($ReInstall -eq 1) {
                &$UnInstall
                &$Loop
                "$date,$time,$env:COMPUTERNAME,SCCM Agent UnInstall Compleate,Information,0" >> "$Logpath\SCCM_Agent_Install.csv"
            }
            else {
                "$date,$time,$env:COMPUTERNAME,Agent reInstall Process Check,Information,0" >> "$Logpath\SCCM_Agent_Install.csv"
            }
        }
        catch {
            "$date,$time,$env:COMPUTERNAME,Error : $($_.Exception.Message), from after Health Check Uninstall Action,20" >> "$Logpath\SCCM_Agent_Install.csv"    
            exit
        }
        ...
        try {
            ...
            "$date,$time,$env:COMPUTERNAME,SCCM Agent reInstall Compleate,Information,0" >> "$Logpath\SCCM_Agent_Install.csv"
        }
        catch {
            "$date,$time,$env:COMPUTERNAME,Error : $($_.Exception.Message), from after ReInstall at UnInstall Action,20" >> "$Logpath\SCCM_Agent_Install.csv"    
            exit
        }
    }
    else {
        "$date,$time,$env:COMPUTERNAME,Asset Scheduled Task Compleate,Information,0" >> "$Logpath\SCCM_Agent_Install.csv"
    }
}  
...
foreach ($SCCMTask in $Task) {
    ...
    $ReturnTask | Add-Member -MemberType NoteProperty `
        ...
        -Value $(if ($SCCMTask.ReturnValue -ne $null) { 
            ...
            "$date,$time,$env:COMPUTERNAME,$($Head[$HeadValue]) Value is $($SCCMTask.ReturnValue),TaskResult,200" >> "$Logpath\SCCM_Agent_Install.csv"
        } 
        else { 'OK' }
    )
