...
Write-Output "Time,Message,Category,ResultCode" > "C:\Temp\$day`_$Comp`_SCCMAgentJob.csv"
Write-Output "$time,Collecting Configuration Manager Installation Info,Information,0" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"
...
foreach ($com in $comp) {
    ...
    if (-not !$source) {
        ...
        if ($CurrentVersion -lt $CompareVersion.build) {
            try {
                Write-Output "$time,CCMsetup.exe File Copy to Temp Folder,information,0" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"
                Start-Sleep -Seconds 2

                try {
                    Start-Process @param -Wait
                    Write-Output "$time,SCCM Agent Install Compleate,Information,0" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"
                }
                catch {
                    Write-Output "$time,Error : $($_.Exception.Message),InstallFail,20" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"    
                    exit
                }
            
            }
            catch {
                Write-Output "$time,Error : $($_.Exception.Message),CopyError,10" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"
                exit
            }
            
        }
        else {
            Write-Output "$time,SCCM Agent Version is Newest ,VersionEnd ,100" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"
        }
    }
    else {
        try {
            Copy-Item \\ad1.dpi.co.kr\NETLOGON\SCCMAgent\ccmsetup.exe c:\temp\
            $param.FilePath = 'C:\Temp\ccmsetup.exe'
            Start-Process @param
            Write-Output "$time,SCCM Agent Install Compleate,Information,0" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"
        }
        catch {
    ...
    if (!$ccmevalLog) {
        ...
        Write-Output "$time,SCCM Agent UnInstall,Information,0" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"
        Start-Process @param
        Write-Output "$time,SCCM Agent Install Compleate,Information,0" >> "C:\Temp\$day`_$env:COMPUTERNAME`_SCCMAgentJob.csv"
    }
}  
